...
class UserProfile extends Component {
  ...
  componentDidMount() {
    if (
      ...
      this.props.getSession(
        `https://api.themoviedb.org/3/authentication/session/new?api_key=${this.props.apiKey}`,
        this.parseRequestToken(this.props.location.search)
      );
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (!nextProps.userDetails.id && nextProps.session.session_id) {
      this.props.getUserDetails(
        `https://api.themoviedb.org/3/account?api_key=${this.props.apiKey}&session_id=${nextProps.session.session_id}`
      );
      this.handleFavorites(
        nextProps.userDetails.id,
        nextProps.session.session_id
      );
  ...
  parseRequestToken = (token) =>
    token.split("?request_token=")[1].split("&")[0];

  handleFavorites = (accountId, sessionId) => {
    this.props.getMovieFavorites(
      `https://api.themoviedb.org/3/account/${accountId}/favorite/movies?api_key=${this.props.apiKey}&session_id=${sessionId}&language=en-US&sort_by=created_at.asc&page=1`
    );
    this.props.getTVFavorites(
      `https://api.themoviedb.org/3/account/${accountId}/favorite/tv?api_key=${this.props.apiKey}&session_id=${sessionId}&language=en-US&sort_by=created_at.asc&page=1`
    );
  };

  handleRated = (accountId, sessionId) => {
    this.props.getMoviesRated(
      `https://api.themoviedb.org/3/account/${accountId}/rated/movies?api_key=${this.props.apiKey}&session_id=${sessionId}&language=en-US&sort_by=created_at.asc&page=1`
    );
    this.props.getTVRated(
      `https://api.themoviedb.org/3/account/${accountId}/rated/tv?api_key=${this.props.apiKey}&session_id=${sessionId}&language=en-US&sort_by=created_at.asc&page=1`
    );
  };

  handleDeleteSession = (sessionId) => {
    this.props.deleteSession(
      `https://api.themoviedb.org/3/authentication/session?api_key=${this.props.apiKey}`,
      sessionId
    );
  ...
  handleGenres = (genresList, genres) => {
    if (genresList) {
      let genresArr = genresList.filter((genre) =>
        genre.id === genres[0] || genre.id === genres[1] ? genre.name : null
      );
      return (
        <p className="user-profile-container-main-container-item-container__genres">
  ...
  userProfileAsideNav = (logInStatus) => {
    if (logInStatus === "APPROVED") {
      return (
        <ul className="user-profile-container-aside-nav-list">
          <li className="user-profile-container-aside-nav-list-item">
            <svg
              className="user-profile-container-aside-nav-list-item__icon"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 512 512"
            >
              <path d="M360 64H152c-22.002 0-40 17.998-40 40v344l144-64 144 64V104c0-22.002-17.998-40-40-40z" />
            </svg>
            <p className="user-profile-container-aside-nav-list-item__title">
              Favorites
            </p>
          </li>
      ...
      return <h1>Must be a TMDB User</h1>;
    }
  };
        ...
        <div className="user-profile-container">
          <aside className="user-profile-container-aside">
            <nav className="user-profile-container-aside-nav">
              <div
                className="user-profile-container-aside-nav-profile wow swing"
                data-wow-delay=".5s"
                data-wow-duration="5s"
              >
                <img
                  className="user-profile-container-aside-nav-profile-image"
                  src={
                    this.props.userDetails.avatar
                      ? `https://secure.gravatar.com/avatar/${this.props.userDetails.avatar.gravatar.hash}.jpg?s=64`
                      : profileAvatar
                  }
                  alt="user"
                />
                <div className="user-profile-container-aside-nav-profile-info">
                  <h2 className="user-profile-container-aside-nav-profile-info__user-name">
                    Welcome{" "}
                    {this.props.userDetails.username
                      ? this.props.userDetails.username
                      : "Guest"}
                  </h2>
                  <p className="user-profile-container-aside-nav-profile-info__user-type">
                    {this.props.userDetails.username ? "TMDB User" : "Guest"}
                  </p>
                </div>
              </div>
              ...
              <Link to="/">
                <button
                  onClick={() =>
                    this.handleDeleteSession(this.props.session.session_id)
                  }
                  className="user-profile-container-aside-nav-log-out wow fadeInUp"
                  data-wow-delay=".5s"
                  data-wow-duration="5s"
                >
                  Log Out
          ...
          <main className="user-profile-container-main">
            <div className="user-profile-container-main-info">
              <div className="user-profile-container-main-info-item">
                <svg
                  className="user-profile-container-main-info-item__image"
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 512 512"
                >
                  <path d="M360 64H152c-22.002 0-40 17.998-40 40v344l144-64 144 64V104c0-22.002-17.998-40-40-40z" />
                </svg>
                <p className="user-profile-container-main-info-item-content">
                  <span className="user-profile-container-main-info-item-content__number">
              ...
              <div className="user-profile-container-main-info-item">
                ...
                <p className="user-profile-container-main-info-item-content">
                  <span className="user-profile-container-main-info-item-content__number">
                    {this.props.moviesRated.results.length}
                  </span>{" "}
                  Rated Movies
                </p>
              </div>
              ...
              <div className="user-profile-container-main-info-item">
                ...
                <p className="user-profile-container-main-info-item-content">
                  <span className="user-profile-container-main-info-item-content__number">
                    {this.props.TVRated.results.length}
                  </span>{" "}
                  Rated TV Shows
                </p>
              </div>
            ...
            <div className="user-profile-container-main-content">
              <div className="user-profile-container-main-container">
                ...
                <div className="user-profile-container-main-container-item">
                  {this.props.movieFavorites.results.length > 0 ? (
                    this.props.movieFavorites.results.map((item) => (
                      <Link key={item.id} to={`/details/movie/${item.id}`}>
                        <div className="user-profile-container-main-container-item-container">
                          <img
                            className="user-profile-container-main-container-item-container__image"
                            src={`${this.props.MDBConfig.images.secure_base_url}${this.props.MDBConfig.images.poster_sizes[1]}${item.poster_path}`}
                            alt={item.title}
                          />
                          <h4 className="user-profile-container-main-container-item-container__title">
                            {item.title}
                          </h4>
                          {this.handleGenres(
                            this.props.movieGenres.genres,
                            item.genre_ids
                          )}
                        </div>
                      </Link>
              ...
              <div className="user-profile-container-main-container">
                ...
                <div className="user-profile-container-main-container-item">
                  {this.props.TVFavorites.results.length > 0 ? (
                    this.props.TVFavorites.results.map((item) => (
              ...
              <div className="user-profile-container-main-container">
                ...
                <div className="user-profile-container-main-container-item">
                  {this.props.moviesRated.results.length > 0 ? (
                    this.props.movieRated.results.map((item) => (
                      <Link key={item.id} to={`/details/movie/${item.id}`}>
                        <div className="user-profile-container-main-container-item-container">
                          <img
                            className="user-profile-container-main-container-item-container__image"
              ...
              <div className="user-profile-container-main-container">
                ...
                <div className="user-profile-container-main-container-item">
                  {this.props.TVRated.results.length > 0 ? (
                    this.props.TVRated.results.map((item) => (
                      <Link key={item.id} to={`/details/tv/${item.id}`}>
                        <div className="user-profile-container-main-container-item-container">
                          <img
                            className="user-profile-container-main-container-item-container__image"
